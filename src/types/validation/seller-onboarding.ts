...
export const farmDetailsSchema = z.object({
  ...
  totalFarmSize: z.string().min(1, "Total farm size is required"),
  
  // Coffee Land Details
  coffeeCoveredLand: z.string().min(1, "Coffee covered land is required"),
  longitude: z.string().min(1, "Longitude is required"),
  latitude: z.string().min(1, "Latitude is required"),
  
  // Crop Environment
  cropType: z.string().min(1, "Crop type is required"),
  cropSource: z.string().min(1, "Crop source is required"),
  origin: z.string().min(1, "Origin is required"),
  cropCapacity: z.string().min(1, "Crop capacity is required"),
  
  // Tree Information
  treeType: z.string().min(1, "Tree type is required"),
  treeVariety: z.string().min(1, "Tree variety is required"),
  
  // Growing Conditions
  averageTemperature: z.string().min(1, "Average temperature is required"),
  soilType: z.string().min(1, "Soil type is required"),
  altitude: z.string().min(1, "Altitude is required"),
  annualRainfall: z.string().min(1, "Annual rainfall is required"),
