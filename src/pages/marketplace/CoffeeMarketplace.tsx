...
const mockListings: CoffeeListing[] = [
  {
    ...
    cup_score: "86.5",
    is_organic: true,
    photo_url: "/placeholder.svg?height=300&width=400",
  },
  {
    id: "2",
...
export default function CoffeeMarketplace() {
  const [listings] = React.useState<CoffeeListing[]>(mockListings)
  const [filteredListings, setFilteredListings] = React.useState<CoffeeListing[]>(mockListings)
  ...
  const [selectedListing, setSelectedListing] = React.useState<CoffeeListing | null>(null)
  ...
  React.useEffect(() => {
    ...
    if (filters.is_organic) {
      const isOrganic = filters.is_organic === "true"
      results = results.filter((item) => item.is_organic === isOrganic)
    }

    if (filters.min_price) {
      results = results.filter((item) => item.price_per_kg >= Number.parseFloat(filters.min_price))
    }

    if (filters.max_price) {
      results = results.filter((item) => item.price_per_kg <= Number.parseFloat(filters.max_price))
    }

    if (filters.min_cup_score) {
      results = results.filter((item) => Number.parseFloat(item.cup_score) >= Number.parseFloat(filters.min_cup_score))
    }
    ...
    <div className="flex flex-col min-h-screen bg-slate-50">
      ...
      <header className="bg-white border-b py-4">
        <div className="container mx-auto px-4 flex items-center justify-between">
          <div className="flex items-center">
            <h1 className="text-2xl font-semibold text-emerald-800">Afrovalley</h1>
          </div>
      ...
      <main className="flex-grow container mx-auto px-4 py-6">
        <div className="mb-6">
          ...
          <p className="text-slate-600 mt-2">Discover premium coffee directly from Ethiopian farmers</p>
        </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Select
                      ...
                      onValueChange={(value) => handleFilterChange("processing_method", value)}
                    >
          ...
          {filteredListings.map((listing) => (
            ...
            >
              <div className="relative h-48 bg-slate-200">
                ...
                />
                {listing.is_organic && <Badge className="absolute top-2 right-2 bg-emerald-500">Organic</Badge>}
              </div>
              <CardContent className="p-4">
                <div className="flex justify-between items-start mb-2">
                  <h3 className="text-lg font-semibold text-slate-800">{listing.coffee_variety}</h3>
                  <div className="flex items-center bg-amber-50 px-2 py-1 rounded">
                    <Star className="h-4 w-4 text-amber-500 mr-1" />
                    <span className="text-sm font-medium text-amber-700">{listing.cup_score}</span>
                  </div>
                </div>
                <p className="text-slate-600 text-sm mb-2">{listing.farm_name}</p>
              ...
              <CardFooter className="px-4 py-3 border-t bg-slate-50 flex items-center justify-between">
                <div className="text-emerald-700 font-bold">${listing.price_per_kg.toFixed(2)}/kg</div>
                <div className="text-slate-500 text-sm">{listing.quantity_kg.toLocaleString()} kg available</div>
              </CardFooter>
            </Card>
          ))}
        </div>
        ...
        {filteredListings.length === 0 && (
          <Card className="p-8 text-center mt-8">
            <CardContent className="pt-6 flex flex-col items-center">
              ...
              <h3 className="text-xl font-semibold text-slate-700 mb-2">No coffee listings found</h3>
              <p className="text-slate-500 mb-4">Try adjusting your filters or search criteria</p>
              <Button onClick={resetFilters} variant="default" className="bg-emerald-600 hover:bg-emerald-700">
                Clear All Filters
              </Button>
            </CardContent>
          </Card>
          ...
          {selectedListing && (
            <>
              <div className="relative">
                ...
                />
                {selectedListing.is_organic && <Badge className="absolute top-4 left-4 bg-emerald-500">Organic</Badge>}
              </div>

              <div className="p-6">
                <div className="flex flex-col md:flex-row md:justify-between md:items-start gap-4 mb-6">
                  <div>
                    <h2 className="text-2xl font-bold text-slate-800">{selectedListing.coffee_variety}</h2>
                    <p className="text-slate-600">{selectedListing.farm_name}</p>
                  ...
                  <div className="flex flex-col items-start">
                    <div className="bg-amber-50 px-3 py-2 rounded-lg flex items-center mb-2">
                      ...
                      <div>
                        <span className="text-lg font-semibold text-amber-700">{selectedListing.cup_score}</span>
                        <span className="text-slate-500 text-sm ml-1">cup score</span>
                      </div>
                    </div>
                    ...
                    <div className="text-slate-600 text-sm">
                      {selectedListing.quantity_kg.toLocaleString()} kg available
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    ...
                    <CardContent>
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          ...
                          <p className="font-medium">{selectedListing.bean_type}</p>
                        </div>
                        <div>
                          <p className="text-sm text-slate-500">Processing</p>
                          <p className="font-medium">{selectedListing.processing_method}</p>
                        </div>
                        <div>
                          <p className="text-sm text-slate-500">Organic</p>
                          <p className="font-medium">{selectedListing.is_organic ? "Yes" : "No"}</p>
                        </div>
                        <div>
                  ...
                  <Card>
                    ...
                    <CardContent className="space-y-4">
                      ...
                      <div>
                        <div className="flex justify-between mb-1">
                          <span className="text-sm text-slate-500">Sweetness</span>
                ...
                <Card className="mb-6">
                  ...
                  <CardContent>
                    <p className="text-slate-700 mb-4">
                      {selectedListing.farm_name} is located in the {selectedListing.region} region of{" "}
