...
export function FileUpload({ onFilesSelected, maxFiles = 5, maxSizeMB = 5, className }: FileUploadProps) {
    ...
    <div className={cn("w-full", className)}>
      <div
        className={cn(
          ...
          isDragging ? "border-primary bg-muted/50" : "border-muted-foreground/25 hover:border-primary/50",
      ...
      {files.length > 0 && (
        <div className="mt-4 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
          {files.map((file, index) => (
            <div key={`${file.file.name}-${index}`} className="relative group border rounded-lg overflow-hidden">
              <div className="aspect-square flex items-center justify-center bg-muted/30">
                {file.type === "image" ? (
                  ...
                  <div className="flex flex-col items-center justify-center p-2">
                    ...
                    <p className="text-xs text-center mt-1 truncate w-full">{file.file.name}</p>
                  </div>
                )}
              </div>
              <button
